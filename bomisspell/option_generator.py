...
def get_yang_jug_options(syl_parts):
    ...
    mingzhi = syl_parts.get('mingzhi', '')
    jes_jug = syl_parts.get('jes_jug', '')
    cur_syl_yang_jug = syl_parts.get('yang_jug', '')
    syl_parts['yang_jug'] = ''
    sngon_jug_options = get_sngon_jug_options(syl_parts)
    options.append(f'{sngon_jug}{mingzhi}{jes_jug}་')
    for yang_jug in ['ད', 'ས']:
        if yang_jug == cur_syl_yang_jug:
            options += sngon_jug_options
            continue
        for sngon_jug_option in sngon_jug_options:
            options.append(f'{sngon_jug_option[:-1]}{yang_jug}་')
    return options  

def get_misspelled_opt(syl_parts):
